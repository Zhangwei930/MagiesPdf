...
| 模式 | 说明 |
| --- | --- |
| **PDF 工作区** | 以标签页打开 PDF；连续滚动、搜索、**高亮与画笔**、脱敏、盖章、填表，运行全部 **61 个工具**，完整撤销 |
...
高亮和墨迹会作为标准的 `/Highlight`、`/Ink` 批注写进文件，因此高亮下面的文字
仍然是文字，任何 PDF 阅读器都能看到、移动或删除它们。一次标注与旋转页面进入
同一条撤销历史，会把文档标记为未保存，并在 `⌘S` 时写入文件。
...
> **macOS：** 未代码签名、未公证，所以双击会提示
> **「"Magies Office" 已损坏，无法打开。您应该将它移到废纸篓。」**
> 文件没有损坏，也不需要删除 —— 任何 macOS 查不到签名的应用都是这句提示，
> 本项目没有购买签名。
>
> 右键 → **打开** → 确认，这条路径才会给你真正的选择。或者在拖入
> 「应用程序」之后执行：
>
> ```bash
...
> ```
>
> Intel 选 **`mac-x64`**，Apple Silicon 选 **`mac-arm64`**。
