...
describe('MCP client configuration', () => {
  it('builds an Electron-as-Node stdio server entry for the running local API', () => {
    const result = buildMcpClientConfig({
      ...
      serverPath: '/Applications/Magies Office.app/Contents/Resources/app.asar/electron/mcp/magies-office-mcp-server.cjs',
    ...
    assert.deepEqual(result.config.mcpServers['magies-office'], {
      ...
      args: ['/Applications/Magies Office.app/Contents/Resources/app.asar/electron/mcp/magies-office-mcp-server.cjs'],
      env: {
